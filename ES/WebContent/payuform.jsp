...
String sucurl="http://localhost:8080/ES/PaymentSuccessController";
String failurl="http://localhost:8080/ES/maintainancehistory.jsp";
%>
<% 	
	String merchant_key="a7kpWo4S";
	String salt="cJVe4wSDpW";
	String action1 ="";
...
<input type="hidden" name="key" value="<%= merchant_key %>" />
      ...
      <table>
        ...
        <tr>
          <td>Amount: </td>
          <td><input name="amount" readonly="readonly" value="<%= (empty(params.get("amount"))) ? "" : params.get("amount") %>" /></td>
          <td>First Name: </td>
          <td><input readonly="readonly" name="firstname" id="firstname" value="<%=userData.getUsername()%>" /></td>
        </tr>
        <tr>
          <td>Email: </td>
          <td><input readonly="readonly" name="email" id="email" value="<%=userData.getEmail_id() %>" /></td>
          <td>Phone: </td>
          <td><input readonly="readonly" name="phone" value="<%=userData.getContact_no()%>" /></td>
        </tr>
        <tr>
          <td>Product Info: </td>
          <td colspan="3"><input readonly="readonly" name="productinfo" value="Maintainance payment" size="64" /></td>
        </tr>
        <tr>
          <td>Success URI: </td>
          <td colspan="3"><input type="hidden" name="surl" value="<%= sucurl%>" size="64" /></td>
        </tr>
        <tr>
          <td>Failure URI: </td>
          <td colspan="3"><input type="hidden" name="furl" value="<%=failurl%>" size="64" /></td>
        </tr>
        <tr>
        ...
        <tr>
          <% if(empty(hash)){ %>
            <td colspan="4"><input type="submit" value="Submit" name="action" /></td>
          <% } %>
        </tr>
      </table>
    </form>
