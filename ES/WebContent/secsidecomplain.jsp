		...
		<form action="" method="post">
			...
			<tbody>
				<tr class="<%=hth.SocietyMember().getTableClass()%>">
													<th style="text-align: center;"><%=HtmlReportHeaderDesign.getReoprtDesign(HtmlTableHeader.secComplaintDetails(),
					ReportConstant.visitorDetails, 0)%></th>
												</tr>
												<%  String socid = dbt.getSocietyID(rg.getEmail_id());
													ArrayList<tran_complain> getComplaints = dbt.getSecComplaintData(socid);
													for (int i = 0; i < getComplaints.size(); i++) {
														tran_complain complain = getComplaints.get(i);
												%>
												<tr>
													<td><%=i+1 %></td>
													...
													<td><a
											href="Society Images/complaintdocs/<%=complain.getDocument()%>"
